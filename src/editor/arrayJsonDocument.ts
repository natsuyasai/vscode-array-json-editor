 ...
 */
export interface ArrayJsonEdit {
  readonly color: string;
  readonly stroke: ReadonlyArray<[number, number]>;
}

export interface ArrayJsonDocumentDelegate {
...
export class ArrayJsonDocument extends Disposable implements vscode.CustomDocument {
  static async create(uri: vscode.Uri, backupId: string | undefined, delegate: ArrayJsonDocumentDelegate): Promise<ArrayJsonDocument | PromiseLike<ArrayJsonDocument>> {
    ...
    return new ArrayJsonDocument(uri, fileData, delegate);
  }
  ...
  private _edits: ArrayJsonEdit[] = [];
  private _savedEdits: ArrayJsonEdit[] = [];
  ...
  private readonly _onDidChangeDocument = this._register(
    new vscode.EventEmitter<{
      ...
      readonly edits: readonly ArrayJsonEdit[];
    }>()
  );
  /**
  ...
  async revert(_cancellation: vscode.CancellationToken): Promise<void> {
    const diskContent = await ArrayJsonDocument.readFile(this.uri);
